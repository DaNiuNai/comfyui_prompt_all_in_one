...
迁移而来，使用两个专用 `STRING` 节点和可拖动、可缩放的 React 悬浮编辑器代替 A1111 固定的正/负提示词输入框。
...
2. 点击节点上的 `Open Prompt Editor` 打开悬浮编辑器；再次点击同一节点按钮可关闭窗口。
3. 在悬浮窗口编辑标签，将节点的 `STRING` 输出连接到对应的文本编码节点。普通画布选择不会切换目标，点击另一个提示词节点的按钮才会显式改绑。
4. 编辑框失焦、显式改绑或关闭窗口时会写入历史；连续相同内容不会重复保存。

节点上的标准多行文本框始终可直接编辑。悬浮窗口保存的禁用状态、译文和排序信息随工作流节点属性一起序列化，实际输出仍是普通字符串。窗口的位置、尺寸和开关状态保存在当前浏览器中，但重载后不会自动恢复目标节点。

### LoRA 与模型检测

悬浮编辑器只检测 `<lora:name:weight>`、`embedding:name` 和 `checkpoint:name` 引用是否存在。ComfyUI 不会像 A1111 一样解析 `<lora:...>` 并自动加载模型；必须在工作流中使用 LoRA Loader 等对应节点。
